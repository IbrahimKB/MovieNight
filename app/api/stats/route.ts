...
export async function GET(req: NextRequest) {
  try {
    ...
    const squadStats = users.map((u) => {
      ...
      const acceptedSuggestions = u.suggestionsFrom.filter(
        (s) => s.status === "accepted",
      ).length;
